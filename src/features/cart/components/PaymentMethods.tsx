...
const PaymentMethods : React.FC<Props> = ({selectedMethod, onChange, onCheckout, loading}) => {
  ...
  const paymentMethods = [
    { id: 1, name: t.payment.cashOnDelivery, logo: null },
    { id: 2, name: "touchngo", logo: touchngo },
  ];

  return (
    <div className="bg-white p-5 rounded-2xl border border-gray-300">
      <h3 className="font-semibold mb-3 border-b border-gray-300 pb-2">{t.payment.title}</h3>
      ...
      <Button disabled={loading || !selectedMethod} onClick={onCheckout} text={loading ? t.payment.processing : t.payment.checkout} className="w-full"/>
    </div>
  );
}
